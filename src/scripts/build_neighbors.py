...
def build_one_hop_neighbors(cfg, click_df: pd.DataFrame, part: str) -> Tuple[Dict, Dict]:
    user_dict = dict()
    news_dict = dict()
    for user, news in click_df.values:
        if user not in user_dict:
            user_dict[user] = []
        if news not in news_dict:
            news_dict[news] = []
        user_dict[user].append(news)
        news_dict[news].append(user)
...
def build_two_hop_neighbors(cfg, user_one_hop: Dict, news_one_hop: Dict, part: str):
    user_dict = dict()
    news_dict = dict()
    for user, news_list in user_one_hop.items():
        two_hop_users = []
    ...
    for news, user_list in news_one_hop.items():
        two_hop_news = []
    ...
    f_news = os.path.join(ROOT_PATH, "data", cfg.fvocab, "{}-news_twoe_hops.txt".format(part))
    with open(f_user, "w", encoding="utf-8") as fw:
        for user, news_list in user_dict.items():
...
def build_click_df(cfg, behavior_df: pd.DataFrame, user_vocab: WordVocab, newsid_vocab: WordVocab) -> pd.DataFrame:
    behavior_df = behavior_df.fillna("")
    click_list = []
    for uid, hist in behavior_df[["uid", "hist"]].values:
    ...
    return pd.DataFrame(click_list, columns=["user", "news"])
...
def main(cfg):
    ...
    train_click_df = build_click_df(cfg, train_behavior, user_vocab, newsid_vocab)
    print("Click pairs in train: {}".format(len(train_click_df)))

    degrees_of_each_user = train_click_df.groupby("user").apply(lambda x: len(x))
    degrees_of_each_news = train_click_df.groupby("news").apply(lambda x: len(x))
    print("User degree stats:\n", degrees_of_each_user.describe())
    print("News degree stats:\n", degrees_of_each_news.describe())
    ...
    test_click_df = build_click_df(cfg, test_behavior, user_vocab, newsid_vocab)
    print("Click pairs in test: {}".format(len(test_click_df)))

    all_click_df = pd.concat([train_click_df, test_click_df], ignore_index=True)
    all_click_df = all_click_df.drop_duplicates(["user", "news"])
    print("Click pairs in all: {}".format(len(all_click_df)))

    # Train one- and two-hops
    train_user_one_hop, train_news_one_hop = build_one_hop_neighbors(cfg, train_click_df, "train")
    build_two_hop_neighbors(cfg, train_user_one_hop, train_news_one_hop, "train")

    # Test one- and two-hops
    test_user_one_hop, test_news_one_hop = build_one_hop_neighbors(cfg, all_click_df, "test")
